...
ANAT="${MAINDIR}/${SUB}/${SUB}_restored-MPRAGE_brain.nii.gz"
cd $SUBDIR
...
# Create streamlines: maxlength=250mm, 1M seeds
tckgen -act 5tt_coreg.mif -algorithm SD_STREAM -seed_gmwmi gmwmseed_atlas.mif -maxlength 250 -select 1000000 wmfod_norm.mif tracks.tck -force
...
#Creating the connectome 
tck2connectome -symmetric -zero_diagonal -scale_invnodevol -tck_weights_in sift.txt tracks.tck atlas.mif "${SUB}_${SES}_connmatrix.csv" -force
