...
for m=1:n_nodes-1
    for n=m+1:n_nodes
        ...
        if any(isnan(x)) || all(x==0)
            compare_anova=[compare_anova;m n 0 0 n_comparisons 0 0];
            continue
        end
...
%         end
        ...
        [~,p,~,stats] = ttest2(hc_mid,mig_inter);
        for idx_p=1:length(c(:,end))
            if p<threshold
                switch c(idx_p,4)>0
                    case 1
                        compare_anova=[compare_anova;m n c(idx_p,1) c(idx_p,2) p*n_comparisons stats.tstat 1];
                    case 0
                        compare_anova=[compare_anova;m n c(idx_p,1) c(idx_p,2) p*n_comparisons stats.tstat -1];
                end
            end
...
table=array2table(compare_anova, "VariableNames", ["Node 1","Node 2","Group 1", "Group 2", "P-value corrected","T statistic","Difference"]);
