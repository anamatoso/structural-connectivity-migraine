function [] = plot_boxplots(metrics,idx,metrics_labels,version_metrics)
n_metrics=length(metrics_labels);

if version_metrics==3
    correction=1;
else
    correction=n_metrics;
end
...
for i=1:length(idx)
    ...
    boxplot(x,g,'Labels',{'HC','M'},'width',0.8)
    title(metrics_labels(index),'interpreter', 'none','FontSize',20,'FontWeight','normal','FontName','Arial')
