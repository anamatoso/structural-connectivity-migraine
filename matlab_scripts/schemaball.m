...
%
	...
	if nargin<5
		positive_color_hs = [0.05 1]; % yellow
		negative_color_hs = [0.6 1]; % magenta
	end
	...
	for ii=idx'
		...
		if corrMatrix(jj,kk)>=0
			clr = hsv2rgb([positive_color_hs 1-abs(corrMatrix(jj,kk))]);
		else
			clr = hsv2rgb([negative_color_hs 1-abs(corrMatrix(jj,kk))]);
		end
		plot(Bt(:,1),Bt(:,2),'color',clr);%,'LineSmoothing','on');
	end
	
	%% draw edge markers of nodes
	...
	for ii=1:M
		text(Px(ii),Py(ii),strNames{ii},'Rotation',theta(ii)*180/pi,'color',0.2*[1 1 1], ...
